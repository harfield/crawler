...
public interface CrawlerMapper {
    ...
    @Select("select * from crawl_attempt a inner join crawl_job b on a.crawlAttemptCrawlJobId=b.crawlJobId where (a.crawlAttemptStat in (0,3,4) and a.crawlAttemptNextCrawlTime<=now()) or (a.crawlAttemptStat=1 and a.crawlAttemptCrawlUpdateTime<date_sub(now(),interval 7200 second)) or (a.crawlAttemptStat=2 and a.crawlAttemptCrawlUpdateTime<date_sub(now(),interval 300 second)) or a.crawlAttemptProducerUpdateTime>a.crawlAttemptSendMqTime order by a.crawlAttemptNextCrawlTime asc,a.crawlAttemptCrawlUpdateTime asc")
    @Results({
